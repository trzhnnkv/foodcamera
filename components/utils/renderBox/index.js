...
export const renderBoxes = async (
  ...
  // font configs
  const font = `${Math.max(Math.round(Math.max(ctx.width, ctx.height) / 40), 14)}pt serif`;
  ctx.font = font;
  ctx.textBaseline = "top";

  const colors = new Colors();

  const desiredClasses = ["Яблоко", "Апельсин", "Банан", "Брокколи", "Морковь"];

  for (let i = 0; i < scores_data.length; ++i) {
    if (scores_data[i] > threshold) {
      const klass = labels[classes_data[i]];

      if (desiredClasses.includes(klass)) {
        ...
        ctx.strokeStyle = Colors.hexToRgba(color, 0.5); // Цвет обводки
        ctx.lineWidth = 7; // Ширина обводки (по вашему выбору)
...
export const getClasses = (
  ...
  for (let i = 0; i < scores_data.length; ++i) {
    if (scores_data[i] > threshold) {
      ...
      if (desiredClasses.includes(klass)) {
        detectedClasses.push(klass);
      }
    }
  }

  let uniqueClasses= [...new Set(detectedClasses)];
